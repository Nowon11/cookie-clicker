(function(){
  // Bookmarklet wrapper to avoid polluting global scope
  'use strict';
  
  // Will get the hostname
  var hostname = "https://www.lschaefer.xyz";
  ...
  Promise.all([
    loadScript(`${hostname}/javascript/jquery.js`),
    loadScript(`${hostname}/javascript/functions.js`)
  ]).then(() => {
    // Dependencies loaded, now initialize multiplayer
    initMultiplayer();
  }).catch((error) => {
    console.error('Failed to load dependencies:', error);
    alert('Failed to load multiplayer dependencies. Please refresh and try again.');
  });
  ...
  function initMultiplayer() {
    ...
    // This will make sure that Cookie Clicker game is loaded before starting everything
    var waitForGame = setInterval(function () {
      if (typeof Game !== "undefined" && Game.ready && document.getElementById("centerArea")) {
  ...
  } // End of initMultiplayer function
  
})(); // End of bookmarklet wrapper
